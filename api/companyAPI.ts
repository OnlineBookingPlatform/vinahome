import type { BusCompanyType, DTO_RP_RegisterSaleTicketOnPlatform, DTO_RQ_RegisterSaleTicketOnPlatform } from "~/types/CompanyType";
...
export const getSaleTicketOnPlatformAPI = async (): Promise<ApiResponse<DTO_RP_RegisterSaleTicketOnPlatform[]>> => {
    const config = useRuntimeConfig();
    const API_GATEWAY_URL = config.public.apiGatewayUrl;
    try {
        return await $fetch<ApiResponse<DTO_RP_RegisterSaleTicketOnPlatform[]>>(`${API_GATEWAY_URL}/v2/company/get-sale-ticket-on-platform`, {
            method: "GET",
        });
    } catch (error) {
        throw error;
